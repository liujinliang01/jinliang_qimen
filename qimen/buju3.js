...
var shier_guiren = new Array("","贵","螣","朱","六","勾","青","空","白","常","玄","阴","后")
...
var dayun_zibai = new Array(10)
var nian_zibai = new Array(10)
var yue_zibai = new Array(10)
var ri_zibai = new Array(10)
var shi_zibai = new Array(10)
var tianpan_guiren = new Array(13)
var renpan_guiren = new Array(13)
var dipan_guiren = new Array(13)
var yuandanpan_guiren = new Array(13)
var lianmu_guiren = new Array(13)
var tianzhi_liuqin = new Array(10)
var dizhi_liuqin = new Array(10)
var renzhi_liuqin = new Array(10)
function bu_sanpan_anzhi_liuqin(rigan){//rigan in Chinese string
	var ref = new Array(2) // ref[0] = wuxing, ref[1] = yinyang
	ref = get_tiangan_wuxing(rigan)
	//alert(ref)
	//alert(ren_anzhi)
	for(var i=1;i<10;i++){
		tianzhi_liuqin[i] = get_liuqin(ref,get_dizhi_wuxing(tian_zhi[i]))
		//alert(get_dizhi_wuxing(ren_anzhi[i]))
		dizhi_liuqin[i] = get_liuqin(ref,get_dizhi_wuxing(di_zhi[i]))
		renzhi_liuqin[i] = get_liuqin(ref,get_dizhi_wuxing(ren_anzhi[i]))		
	}
	//alert(renzhi_liuqin)
}
function bu_sanpan_guiren(d,h){ // d is 日干 in Chinese; h is 几点钟, in 24h format
	bu_yuandanpan_guiren(d,h)
//alert(yuandanpan_guiren)
	var temp
	for(var i=1;i<13;i++){
		temp = locate(tian_zhi[i],shier_dizhi)
		//alert(temp)
		tianpan_guiren[i] = yuandanpan_guiren[temp]

		temp = locate(di_zhi[i],shier_dizhi)
		dipan_guiren[i] = yuandanpan_guiren[temp]

		temp = locate(ren_anzhi[i],shier_dizhi)
		renpan_guiren[i] = yuandanpan_guiren[temp]
	}
}

function bu_yuandanpan_guiren(day_gan,hour){//day_gan是当日天干，hour是正时24小时制

	var danmu
	if(hour>5 || hour < 17){//用卯酉为分界
		danmu = 1
	} else {
		danmu = -1
	}

	var a = new Array(2) // this will store two values, a[0]=贵人起点,a[1]=贵人顺逆
...
*/
	...
	a[0] = locate(temp.valueOf(),shier_dizhi)

	for(var i = 1; i<13;i++){
		if(a[1]>0){
		//alert("顺")
			yuandanpan_guiren[cycle(i+a[0]-1,12)] = shier_guiren[cycle(i,12)]
		}else{
			yuandanpan_guiren[cycle(-i+a[0]+1,12)] = shier_guiren[cycle(-i,12)]	
			//alert("逆")
		}
...
function bu_all_zibai(){
var ju = 1 // all the five zibai should be 顺, so we will just use a positive number  

	var dayun_ruzhong = get_dayun_zibai_ruzhong(nian)
	bu_zibai(ju,dayun_ruzhong,dayun_zibai)

	var nian_ruzhong = nian_zibai_ruzhong(nian)
	bu_zibai(ju,nian_ruzhong,nian_zibai)
	
	var nianzhi = c.yGz_Str()
	nianzhi = nianzhi.substr(1)
	var yinli = c.lunar()
	var yue_ruzhong = yue_zibai_ruzhong(nianzhi,yinli.lunarMonth)
	bu_zibai(ju,yue_ruzhong,yue_zibai)

	var ri_ruzhong = ri_zibai_ruzhong(c)
	bu_zibai(ju,ri_ruzhong,ri_zibai)

	var shi_ruzhong = shi_zibai_ruzhong(ju, c.dGz_Str(), c.hGz_Str())
	bu_zibai(ju,shi_ruzhong,shi_zibai)
//alert(shi_zibai)
}
...
// 两个array被赋值： 一个储存地盘各宫得何干，另一个储存旬首、年、月、日、时、值符各在何宫
	...
	if (seed_zhu_xushu%10 == 1 || seed_zhu_xushu%11 == 5){ //用以起局的种子时柱如果是六甲旬首时，括囊要求天盘直符加在地盘戊，而不是加在地盘旬首上
		di_info[5] = Math.abs(jushu)
	}
	//alert(di_info)
	zhishi = find_zhishi(di_info[0],cycle(seed_zhu_xushu,10))
	if (seed_zhu_xushu%10 == 1 || seed_zhu_xushu%11 == 5){ //用以起局的种子时柱如果是六甲旬首时，括囊要求致使门加在暗干戊所在之宫
...
function yue_zibai_ruzhong(nianzhi,yueshu) {//nianzhi is the Chinese str, like "午"; yueshu is the lunar month in Arabic number
...
function shi_zibai_ruzhong(ju, dgz, hgz) {//dgz is the Chinese string of 日干支; hgz is the Chinese string for 时干支
